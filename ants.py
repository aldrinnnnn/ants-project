...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        counter = 0
        while counter < self.min_range and not current_place.is_hive:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        reduction = amount
        if self.health <= 0:
            reduction += self.damage
        for bee in list(place.bees):
            bee.reduce_health(reduction)
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.scare_count > 0 and destination != self.place:
            self.scare_count -= 1
